...
def cat(file_path,year,gdp_option):
  ...
  for i in partition_tables:
    ...
    map_list.append(map_lowest_10_gdp(partition=y,year=year,gdp_opt=gdp_option))
    #map_list.append(map_max_gdp(partition=y,year=2019,gdp_opt='GDP_USD'))
    
  return map_list


def map_lowest_10_gdp(partition,year,gdp_opt):
    key_list=[]
    partition=partition[partition['year']==year]
    low_df=partition.sort_values(by=[gdp_opt])
    for i in range(10):
        key_list.append([low_df.sort_values(by=[gdp_opt]).iloc[i]['Country_Name'],low_df.sort_values(by=[gdp_opt]).iloc[i][gdp_opt]])
    if 'map' in explain:
...
def combiner_lowest_10_gdp(map_list):
    dict_lowest={}
...
def reduce_lowest_10_gdp(dict_lowest):
...
def min_main(file_path,year,gdp_option):
    ...
    combined_res=combiner_lowest_10_gdp(map_list)
    return explain,pd.DataFrame(reduce_lowest_10_gdp(combined_res[0]),columns=['Country with minimum {} for year {}'.format(gdp_option,year)])

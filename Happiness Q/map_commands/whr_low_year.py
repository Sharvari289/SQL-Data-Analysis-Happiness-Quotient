...
def cat(file_path,whr_col,country):
  ...
  for i in partition_tables:
    ...
    map_ls.append(map_lowest_score_year(y,whr_col,country))
    
  return map_ls


def map_lowest_score_year(partition,whr_col,country):
    key_list=[]
...
def reduce_lowest_score_year(map_ls):
    min_val=10**20
    ...
    for val in map_ls:
        ...
        if val[1]<min_val:
            min_val=val[1]
            min_year=val[0]
    explain['reducer']=[min_year,min_val]
    return [min_year,min_val]
        
def low_year_main(file_path,whr_col,country):
    map_ls=cat('users/pop2/pop3/whr.csv',whr_col,country)
    #dict_ls=combiner_lowest_10_whr(map_ls)
    return explain,pd.DataFrame([reduce_lowest_score_year(map_ls)],columns=['Minimum year','Minimum value for {} for country {}'.format(whr_col,country)])
...
def cat(file_path,year,gdp_option):
  ...
  for i in partition_tables:
    ...
    map_list.append(map_max_gdp(partition=y,year=year,gdp_opt=gdp_option))
    #map_list.append(map_max_gdp(partition=y,year=2019,gdp_opt='GDP_USD'))
    
  return map_list


def map_max_gdp(partition,year,gdp_opt):
    key_list=[]
    partition=partition[partition['year']==year]
    key_list.append(partition.sort_values(by=[gdp_opt],ascending=False).iloc[0]['Country_Name'])
    key_list.append(partition.sort_values(by=[gdp_opt],ascending=False).iloc[0][gdp_opt])
    value=1
...
def reduce_max_gdp(map_list):
    ...
    for val in map_list:
        ...
        if gdp>max_val:
            max_val=gdp
            max_country=val[0][0]
    explain['reducer']=[max_country,max_val]
    return max_country,max_val  

def max_main(file_path,year,gdp_option):

    map_list=cat('/GDP_updated.csv',year,gdp_option)
    return explain,pd.DataFrame([reduce_max_gdp(map_list)],columns=['Country with maximum {}'.format(gdp_option),'Maximum value for {}'.format(year)])
